...
class Store extends EventEmitter {
  ...
  projectSummary() {
    ...
    const add = (c, archived) => {
      ...
      if (!row.repoUrl && c.repoUrl) row.repoUrl = c.repoUrl;
    };
  ...
  noteSkippedBefore(id, info) {
    ...
    const commands = Array.isArray(info.commands)
      ? info.commands.filter((c) => typeof c === 'string' && c).slice(0, 5)
      : [];
    card.skippedBefore = {
      count: count,
      commands: commands,
  ...
  noteSkipCommand(id, command) {
    ...
    // reachable by anything on loopback, and the name becomes a key persisted
    // in board.json and rendered as a tag. Hold it to the shape a slash command
    // actually has rather than trusting the caller.
    ...
    if (!name || name.length > 60 || !/^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(name)) return null;
    const now = nowIso();
